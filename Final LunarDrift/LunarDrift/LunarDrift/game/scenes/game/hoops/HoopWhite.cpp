	...
	: SceneObject(context, managers)
{
	...
	m_Texture = std::make_unique<Texture>(context, "HoopWhite.dds");
...
void HoopWhite::predraw(Context* context)
{
	SceneObject::predraw(context);
}

void HoopWhite::draw(Context* context, std::shared_ptr<ShaderManager> shaders)
{
...
void HoopWhite::onMessage(std::shared_ptr<Message> event)
{
	if (event->getType().compare("Collision") == 0)
	{
		...
		Ship* temp = dynamic_cast<Ship*>(so);
		if (temp != nullptr)
		{
			if (!crossed)
			{
				...
				std::shared_ptr<ScoreAddMessage> sam =
					std::make_shared<ScoreAddMessage>(temp->isPlayer2Controlled(), 10);
				MessageSystem::getInstance().broadcast(sam);

				std::shared_ptr<SceneActivateMessage> sm =
					std::make_shared<SceneActivateMessage>("Settings");
				MessageSystem::getInstance().broadcast(sm);
			}
		}
	}
} 