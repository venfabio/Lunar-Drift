	...
	: SceneObject(context, managers)
{
	...
	m_Texture = std::make_unique<Texture>(context, "HoopYellow.dds");
...
void HoopYellow::predraw(Context* context)
{
	SceneObject::predraw(context);
}

void HoopYellow::draw(Context* context, std::shared_ptr<ShaderManager> shaders)
{
...
void HoopYellow::onMessage(std::shared_ptr<Message> event)
{
