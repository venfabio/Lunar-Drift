...
Derpy::Derpy(
	...
	bool isHardMode,
	bool isPlayer2)
	: Ship(context, managers, position, 100, isHardMode, isPlayer2)
{
	...
	points.push_back({ 1, glm::vec3(0, 0.0, 0.65), 0.35 });
	points.push_back({ 2, glm::vec3(0, 0.0, 0.2), 0.25 });
	points.push_back({ 3, glm::vec3(0, 0.0, -0.3), 0.25 });
	points.push_back({ 4, glm::vec3(0.25, 0.0, -0.125), 0.15 });
	points.push_back({ 5, glm::vec3(-0.25, 0.0, -0.125), 0.15 });
	points.push_back({ 6, glm::vec3(0.25, 0.0, 0.4), 0.15 });
	points.push_back({ 7, glm::vec3(-0.25, 0.0, 0.4), 0.15 });

	setupJets(context, managers);
	addComponent(std::make_shared<PhysicsComponent>(this, managers, 100.f));
	addComponent(std::make_shared<CollisionComponent>(this, managers, 1.0f, points));

	m_Model = std::make_shared<Model>(context, "Derpy");
	m_Texture = std::make_unique<Texture>(context, "TextureC1.dds");

	//setupOutline(context);
}

void Derpy::setupJets(Context* context, std::weak_ptr<MetaManager> managers)
{
...
void Derpy::setupOutline(Context* context)
{
...
void Derpy::predraw(Context* context)
{
	Ship::predraw(context);
}

void Derpy::draw(Context* context, std::shared_ptr<ShaderManager> shaders)
{
...
void Derpy::onMessage(std::shared_ptr<Message> event)
{
	...
	else if (event->getType().compare("ButtonDown") == 0)
	{
		...
		if (bde->getInputName().compare("Fire") == 0 && m_CurrentPowerUp != PowerUps::None)
		{
			std::shared_ptr<SceneActivateMessage> sam =
				std::make_shared<SceneActivateMessage>("Fire");
