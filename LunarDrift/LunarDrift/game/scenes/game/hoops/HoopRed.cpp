	...
	: SceneObject(context, managers)
{
	...
	m_Texture = std::make_unique<Texture>(context, "HoopRed.dds");
...
void HoopRed::predraw(Context* context)
{
	SceneObject::predraw(context);
}

void HoopRed::draw(Context* context, std::shared_ptr<ShaderManager> shaders)
{
...
void HoopRed::onMessage(std::shared_ptr<Message> event)
{
